...
@section('title', 'Editar sección de medios')
...
<div style="margin-bottom:24px;">
    <h1 style="font-size:22px; font-weight:600; color:var(--gray-800); margin:0 0 4px;">Editar sección de medios</h1>
    <p style="font-size:13px; color:var(--gray-400); margin:0;">
        Modificando: <strong>{{ $medio->tipoLabel() }}</strong>
    </p>
</div>

<form method="POST"
      action="{{ route('admin.medios.update', $medio) }}"
        ...
        <div class="card-body">
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="tipo"
                        ...
                        @foreach($tipos as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('tipo', $medio->tipo) === $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group full">
                    ...
                    <textarea name="texto"
                              class="form-control @error('texto') is-invalid @enderror">{{ old('texto', $medio->texto) }}</textarea>
    ...
    <div class="card" style="margin-bottom:16px;">
        ...
        <div class="card-body">
            <p style="font-size:12px; color:var(--gray-400); margin:0 0 14px;">
                Marca las que quieras eliminar y guarda el formulario.
            </p>
    ...
    <div class="card" style="margin-bottom:16px;">
        <div class="card-header">
            <h2>{{ $medio->imagenes->count() ? 'Añadir más imágenes' : 'Imágenes' }}</h2>
        </div>
        <div class="card-body">
            ...
            @error('imagenes.*')
                <span class="form-error" style="display:block; margin-top:6px;">{{ $message }}</span>
            @enderror
        </div>
    </div>
    ...
    <div class="card" style="margin-bottom:24px;">
        ...
        <div class="card-body">
            <p style="font-size:12px; color:var(--gray-400); margin:0 0 14px;">
                Al guardar se reemplaza la tabla completa con lo que veas aquí.
            </p>
            ...
            <div id="enlaces-container">
                @php
                    $enlacesData = old('enlaces')
                        ? collect(old('enlaces'))->map(fn($e) => (object)$e)
                        : $medio->enlaces;
                @endphp

                @foreach($enlacesData as $i => $enlace)
                <div class="enlace-row"
                     style="display:grid; grid-template-columns:1fr 1fr 36px;
                            gap:8px; margin-bottom:8px; align-items:center;">
                    <input type="text"
                           name="enlaces[{{ $i }}][nombre]"
                           value="{{ is_object($enlace) ? $enlace->nombre : ($enlace['nombre'] ?? '') }}"
                           placeholder="Ej. Best Paper Award 2023"
                           class="form-control" style="margin-bottom:0;">
                    <input type="url"
                           name="enlaces[{{ $i }}][url]"
                           value="{{ is_object($enlace) ? $enlace->url : ($enlace['url'] ?? '') }}"
                           placeholder="https://..."
                           class="form-control" style="margin-bottom:0;">
...
// ── Filas de enlaces dinámicas ────────────────────────────────────────────────
(function () {
    let idx = {{ old('enlaces') ? count(old('enlaces')) : $medio->enlaces->count() }};
